...
client.on('message', async (topic, message) => {
  ...
  console.log('Processing: '+ path);
    ...
    await onCommand(topic, message);
  }
});

// Questions topics
// questions/<collaborationId>/load?json_path=$.context.questions
// questions/<collaborationId>/save/<questionId>  - save question
// questions/<collaborationId>/answered - answered questions
...
function getPaths(topic, param) {
  // Parse the topic URL style
  // Format: "save/my/data/here?json_path=$.phoneNumbers[:1].type"
  let json_path = null;

  // Check if topic contains query parameters
  const [basePath, queryString] = topic.split('?');

  // Extract json_path from query parameters if present
  if (queryString) {
    const params = new URLSearchParams(queryString);
    json_path = params.get(param);
  }
  return {json_path, basePath};
}
